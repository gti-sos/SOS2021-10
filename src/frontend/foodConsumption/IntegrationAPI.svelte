<script>
	...
	var BASE_CONTACT_API_PATH= "https://sos2021-10.herokuapp.com/api/integration";
	...
	async function getFoodconsumption(){
		...
		const res = await fetch(BASE_CONTACT_API_PATH + "/foodconsumption-stats?offset="+c_offset+"&limit=" + limit);
	...
	async function getFiltro(){
		...
		const res = await fetch(BASE_CONTACT_API_PATH + "/foodconsumption-stats" + dbquery);
	...
	async function loadInitialData(){
		console.log("Fetching foodconsumption...");
		const res = await fetch(BASE_CONTACT_API_PATH + "/foodconsumption-stats/loadInitialData").then( (res)=> {
						getFoodconsumption();
						if(res.ok){
			...
			<div>
      		...
      		<Pagination ariaLabel="Web pagination">
        		<PaginationItem class = {c_page === 1 ? "disabled" : ""}>
          			<PaginationLink previous href="#/foodconsumption-stats/IntegrationAPI" on:click={() => cambiapag(c_page - 1, c_offset - 10)}/>
        		</PaginationItem>
        		{#each range(lastPage, 1) as page}
          			<PaginationItem class = {c_page === page ? "active" : ""}>
            			<PaginationLink previous href="#/foodconsumption-stats/IntegrationAPI">
            				{page}
            			</PaginationLink>
          			</PaginationItem>
        		{/each}
        		<PaginationItem class = {c_page === lastPage ? "disabled" : ""}>
          			<PaginationLink next href="#/foodconsumption-stats/IntegrationAPI" on:click={() => cambiapag(c_page + 1, c_offset + 10)}/>
        		</PaginationItem>
      		</Pagination>
